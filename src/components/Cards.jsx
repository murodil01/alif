...
const Cards = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-4 py-10">
      {products.map((item) => (
        <div key={item.id} className="bg-white rounded-xl shadow-md p-4 relative group">
          <img src={item.image} alt={item.title} className="w-full h-48 object-contain mb-4" />
          <h3 className="text-gray-800 font-semibold text-lg mb-2 line-clamp-2">{item.title}</h3>
          <div className="flex items-center gap-2">
            <span className="text-red-500 font-bold text-lg">${item.price}</span>
            <span className="line-through text-gray-400 text-sm">${(item.price * 1.2).toFixed(2)}</span>
          </div>
